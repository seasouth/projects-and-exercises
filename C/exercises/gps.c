...
void readCity( FILE *fp, struct City *cty ) {
  ...
  // ...
  fscanf(fp, "%s%lf%lf", cty->name, &cty->latitude, &cty->longitude);
...
int main( int argc, char *argv[] )
{
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    // ...
    struct City nearestCity;
    double distance = 0;
    for (int i = 0; i < n; i++) {
        double currentDistance = 
            globalDistance(lat, lon, cities[i].latitude, cities[i].longitude);
        if (currentDistance < distance || distance == 0) {
            distance = currentDistance;
            nearestCity = cities[i];
        }
    }
    printf("%s %.2lf\n", nearestCity.name, distance);
  }
